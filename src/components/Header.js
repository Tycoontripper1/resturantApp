...
export const Header = () => {
  ...
  const [{user, cartShow, cartItems}, dispatch]=UseGlobalContext()
  ...
  return (
    <header className='fixed z-50 w-screen p-3 px-4  md:p-6 md:px-16 bg-primary drop-shadow-md '>
        ...
        <div className='hidden md:flex w-full h-full items-center justify-between '>
           ...
           <motion.img whileTap={{scale : 0.6}} src={user? user.photoURL : Avatar } className="w-10  h-10 min-w-[40px] min-h-[40px] drop-shawdow-xl rounded-full" alt='avatar' onClick={login}/> 
           ...
           {isMenu && (
            ...
            exit={{opacity :0, scale: 0.6}} 
             className='w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0 px-4 py'>
            {user && user.email === "oayodeji27@gmail.com" && ( 
             <Link to={'/createItem'}>
                  <p onClick={()=> setIsMenu(false)} className='px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base '>New Item <MdAdd /> </p>
             </Link>
            ) }
        ...
        <div className='relative flex items-center justify-center' onClick={showCart}>
           ...
           {isMenu && (
            ...
            {user && user.email === "oayodeji27@gmail.com" && ( 
             ...
             <p className='m-2 p-2 px-4 py-2 flex justify-center rounded-md shadow-md  items-center gap-3 bg-gray-200 cursor-pointer hover:bg-gray-300 transition-all duration-100 ease-in-out text-textColor text-base ' onClick={logout}>Logout <MdLogout /></p>
           </motion.div>
           ) }
